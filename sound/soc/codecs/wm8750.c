...
static const char *wm8750_micboot_sel[] = {"Boost off", "13dB","20dB","29dB"};
...
SOC_ENUM_SINGLE(WM8750_ADCIN, 6, 4, wm8750_micboot_sel), /* 17 */
...
SOC_ENUM("Left Right Mic boost", wm8750_enum[17]),
...
static const struct snd_soc_dapm_widget wm8750_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MUX("ROUT2", SND_SOC_NOPM, 0, 0, &wm8750_snd_controls[41]),
...
#if 0
int wm8750_headset_detect(struct snd_soc_codec *codec, int jack_insert)
{
       int jack_type=0;
       u16 pwr_reg=0;

       if (jack_insert) {
              printk("alex:codec:wm8750:func=%s jack inserted\n",__func__);
              pwr_reg=snd_soc_read(codec, WM8750_PWR1);
               snd_soc_write(codec, WM8750_PWR1, pwr_reg | 0x00fe);
               snd_soc_write(codec, WM8750_PWR2, 0x1e0);
               snd_soc_write(codec, WM8750_ADCTL1, 0x0c8);
               snd_soc_write(codec, WM8750_ADCTL2, 0x003);
              jack_type = WM8750_HEADSET_DET;
       } else {
              printk("alex:codec:wm8750:func=%s jack not inserted\n",__func__);
               snd_soc_write(codec, WM8750_PWR2, 0x000);
               snd_soc_write(codec, WM8750_ADCTL1, 0x0c0);
               snd_soc_write(codec, WM8750_ADCTL2, 0x003);
               snd_soc_write(codec, WM8750_PWR1, 0x0c0);
       }
       return jack_type;
}
EXPORT_SYMBOL(wm8750_headset_detect);
#endif
	...
	if (coeff >= 0){
		snd_soc_write(codec, WM8750_SRATE, srate |
			(coeff_div[coeff].sr << 1) | coeff_div[coeff].usb);
               srate = snd_soc_read(codec, WM8750_SRATE);
                printk("%s srate is %x\n", __func__, srate);
        }
				 ...
				 enum snd_soc_bias_level level)
{
	u16 pwr_reg = snd_soc_read(codec, WM8750_PWR1) & 0xfe3e;
        ...
        u16 adcdac_reg = snd_soc_read(codec, WM8750_ADCDAC);
//        printk("%s\n", __func__);
	switch (level) {
...
//                printk("%s SND_SOC_BIAS_ON\n", __func__);
                ...
                snd_soc_write(codec, WM8750_PWR1, pwr_reg | 0x00fe);
                snd_soc_write(codec, WM8750_PWR2, 0x1f8);

                snd_soc_write(codec, WM8750_ADCTL1, 0x0C8);
                snd_soc_write(codec, WM8750_ADCTL2, 0x003);
                snd_soc_write(codec, WM8750_ADCDAC, 0x1ee);

                /*Vol Cntl*/
                snd_soc_write(codec, WM8750_LOUT2V, 0x17f);
                snd_soc_write(codec, WM8750_ROUT2V, 0x17f);
                snd_soc_write(codec, WM8750_LDAC, 0x1ff);
                snd_soc_write(codec, WM8750_RDAC, 0x1ff);
                /*micboost*/
               	snd_soc_write(codec, WM8750_LADCIN, 0x130);
               	snd_soc_write(codec, WM8750_RADCIN, 0x130);

                /*DAC Cntl*/
                snd_soc_write(codec, WM8750_LOUTM1, 0x100);
                snd_soc_write(codec, WM8750_LOUTM2, 0x100);
                snd_soc_write(codec, WM8750_ROUTM1, 0x100);
                snd_soc_write(codec, WM8750_ROUTM2, 0x100);
                /*mic to hp*/
//                snd_soc_write(codec, WM8750_ROUTM2, 0x080);
//                snd_soc_write(codec, WM8750_LOUTM2, 0x080);

		/* Unmute DAC at end to avoid pop noise */
        	adcdac_reg = snd_soc_read(codec, WM8750_ADCDAC);
                snd_soc_write(codec, WM8750_ADCDAC, adcdac_reg & 0xfff7);
...
//                printk("%s SND_SOC_BIAS_STANDBY\n", __func__);
		...
		/* mute dac */ 
        	adcdac_reg = snd_soc_read(codec, WM8750_ADCDAC);
                snd_soc_write(codec, WM8750_ADCDAC, adcdac_reg | 0x0008);

                snd_soc_write(codec, WM8750_LOUT2V, 0x17f);
                snd_soc_write(codec, WM8750_ROUT2V, 0x17f);
		/* set vmid to 500k, enable VREF and disable master clock */
		snd_soc_write(codec, WM8750_PWR1, pwr_reg | 0x0140);
		snd_soc_write(codec, WM8750_PWR2, 0x078);
		msleep(1);
		snd_soc_write(codec, WM8750_PWR1, pwr_reg | 0x141);
		break;
...
#define WM8750_REG_DISP_LEN 23
static ssize_t wm8750_index_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
       ...
       cnt += sprintf(buf, "WM8750 index register\n");
       for (i = 0; i < WM8750_CACHE_REGNUM; i++) {
              if (cnt + WM8750_REG_DISP_LEN >= PAGE_SIZE)
                     break;
              val = snd_soc_read(codec, i);
              cnt += snprintf(buf + cnt, WM8750_REG_DISP_LEN,
              "%02x(%d): %04x\n", i,i,val);
       }
...
static DEVICE_ATTR(index_reg, 0444, wm8750_index_show, NULL);
...
#endif
       snd_soc_write(codec, WM8750_RESET, 0);
