...
{
	pr_info("%s(): Initialzing BTLSR SCO Codec\n", __func__);
...
{
	pr_info("%s(): Unregister BTLSR SCO Codec\n", __func__);
...
static int __init bt_sco_init(void)
{
	return platform_driver_register(&bt_sco_driver);
}
late_initcall(bt_sco_init);
