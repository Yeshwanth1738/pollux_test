...
static void __init imx6q_csi_mux_init(void)
{
	...
	/* ESOMIMX6 ANKAA QUAD
	 * IPU1 CSI0 connects to parallel interface (by default)
	 * Set GPR1 bit 19 to 0x1
	 * In case, if ov8825 mipi is connected, IPU1 CSI0 connects to MIPI interface (uboot automatically detects it)
	 * 		In this case, set GPR1 bit 19 to 0x0
	 *
	 ...
	 * ESOMIMX6 ANKAA SOLO
	 * IPU1 CSI0 connects to parallel interface
	 * Set GPR13 bits (2-0) to '100' (binary)
	 * IPU1 CSI1 connects to MIPI interface
	 * Set GPR 13 bits (5-3) tp '000' (binary)
	 */
	struct regmap *gpr;
	...
	if (!IS_ERR(gpr)) {
		...
		else if (of_machine_is_compatible("fsl,imx6q-ankaa")) {
			regmap_update_bits(gpr, IOMUXC_GPR1, 1 << 19, 0 << 19);
			regmap_update_bits(gpr, IOMUXC_GPR1, 1 << 20, 1 << 20);
		}
		
		else if (of_machine_is_compatible("fsl,imx6q-ankaa-ov8825") ) {
			regmap_update_bits(gpr, IOMUXC_GPR1, 1 << 19, 0 << 19);
			regmap_update_bits(gpr, IOMUXC_GPR1, 1 << 20, 1 << 20);
		}
		
		else if (of_machine_is_compatible("fsl,imx6dl-ankaa")) {
			regmap_update_bits(gpr, IOMUXC_GPR13, 0x3F, 0x04);
		}

		/* Add support for eSOMiMX6 ANKAA */
		else if (of_machine_is_compatible("fsl,imx6q-ankaa"))
		{
			regmap_update_bits(gpr, IOMUXC_GPR1, 1 << 19, 1 << 19);
			regmap_update_bits(gpr, IOMUXC_GPR1, 1 << 20, 1 << 20);
