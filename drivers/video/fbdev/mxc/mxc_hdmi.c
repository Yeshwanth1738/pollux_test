...
static struct fb_videomode initial_mode = {
	NULL, 69, 640, 400, 39721, 40, 24, 39, 9, 96, 2, 0,
	0, FB_MODE_IS_FROM_VAR,
};
...
static void mxc_hdmi_set_mode(struct mxc_hdmi *hdmi)
{
	...
	if (!hdmi->dft_mode_set) {
		if(!memcmp(&initial_mode,&hdmi->default_mode,sizeof(initial_mode)))
			hdmi->default_mode=mxc_cea_mode[DEFAULT_VIDEO_MODE];
