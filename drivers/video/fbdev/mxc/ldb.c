...
bool ldb_dual_mode=false;
bool ldb_split_mode=false;

static int g_ldb_mode;
static int g_hdmi_mode = 1;
 ...
 *    at the same time.
 */
static int __init ldb_setup_e_con(char *options)
{
	g_ldb_mode = parse_ldb_mode(options);

	if ((g_ldb_mode == LDB_SPL_DI0) || (g_ldb_mode == LDB_SPL_DI1))
		ldb_split_mode=true;

	else if ((g_ldb_mode == LDB_DUL_DI0) || (g_ldb_mode == LDB_DUL_DI1))
		ldb_dual_mode=true;

#if 0
	if ((g_ldb_mode == LDB_SIN0) || (g_ldb_mode == LDB_SIN1)) 
		is_primary=true;
	else
		is_primary=false;
#endif	
...
//	ldb->dual_mode = of_property_read_bool(np, "dual-mode");
	...
	for_each_child_of_node(np, child) {
		...
		/* e-Con Systems : Added to make the LVDS 
		 * kernel argument as the primary LVDS screen
		 */
		...
		if(g_hdmi_mode){
			...
			if ((chan->chno == 0) && (g_ldb_mode == LDB_SIN0 ))
				is_primary = true ;
			else if ((chan->chno == 1) && (g_ldb_mode == LDB_SIN1 ))
				is_primary = true ;
			else if ((chan->chno == 0) && (g_ldb_mode == LDB_SEP0) || (chan->chno == 1) && (g_ldb_mode == LDB_SEP1))
				is_primary = true ;
			else
				is_primary=false;
		}
		...
		/********lcd_disaply****/
		if(!fb_get_options_vie("mxcfb0", "800x480M") || !fb_get_options_vie("mxcfb1", "800x480M"))
			ind=0;/*select the 800x480M node in the device tree*/
		else if(!fb_get_options_vie("mxcfb0", "LDB-XGA") || !fb_get_options_vie("mxcfb1", "LDB-XGA"))
			ind=1;/*select the hsd100pxn1 node in the device tree*/

		ret = of_get_videomode(child, &chan->vm, ind);
		if (ret)
			return -EINVAL;
