...
static ssize_t touch_accessed_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct ads7846 *ts = dev_get_drvdata(dev);
...
static ssize_t touch_accessed_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ts->touch_access = (bool)data;
	rc =count;
	return rc;
}
static DEVICE_ATTR_RW(touch_accessed);
//static DEVICE_ATTR(touch_accessed, S_IRUGO, touch_accessed_show, NULL);
...
static irqreturn_t ads7846_irq(int irq, void *handle)
{
	...
	if(!ts->touch_access && ts->gpio_pendown)
	{
		ts->touch_access = 1;				
	}
