	...
	uint32_t pixel_fmt, ipu_csi_signal_cfg_t cfg_param)
{
	...
	case IPU_PIX_FMT_Y10:
	case IPU_PIX_FMT_Y12:
	...
	case IPU_PIX_FMT_RGB555:
		...
		break;
	case IPU_PIX_FMT_JPEG:
		cfg_param.data_fmt = CSI_SENS_CONF_DATA_FMT_JPEG;
#if 0
		data |= (1 << CSI_SENS_CONF_JPEG8_EN_SHIFT);
		data |= (1 << CSI_SENS_CONF_JPEG_EN_SHIFT);
		printk("\nIPU_PIX_FMT_JPEG : Configured in the ipu CSI_SENS_CONF data = 0x%x \n",data);
#endif
		break;
	default:
		return -EINVAL;
