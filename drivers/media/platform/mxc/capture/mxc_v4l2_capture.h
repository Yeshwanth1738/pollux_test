...
#include "../drivers/media/i2c/aptina-pll.h"
...
typedef struct _mt9p031_register_info {
	u8 reg;
	u16  val;
} mt9p031_register_info;
...
struct sensor_data {
	...
	int brightness;
	int hue;
	int noise_filter;//for 3MP camera
	int stats_exposure;//for 3MP camera
	int contrast;
	int saturation;
	int aw_mode;
	int wb_temp;
	...
	int scene_mode;
	int mode2;
	mt9p031_register_info register_info;
	u16 output_control;
	int reset;
	enum mt9p031_model model;
	struct v4l2_control *blc_auto;
	struct v4l2_control *blc_offset;
