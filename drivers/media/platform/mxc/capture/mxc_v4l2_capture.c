...
#define V4L2_CID_TEMP_SENSE_AR1820_CTRL		(V4L2_CID_DRIVER_BASE + 2)
#define V4L2_CID_TEMP_SENSE_AR1820_READ		(V4L2_CID_DRIVER_BASE + 3)
#define V4L2_CID_NOISE_FILTER			(V4L2_CID_DRIVER_BASE + 4)

#define V4L2_CID_EXPOSURE_ROI_WINDOW		(V4L2_CID_DRIVER_BASE + 5)
#define V4L2_CID_EXPOSURE_ROI			(V4L2_CID_DRIVER_BASE + 6)
#define V4L2_CID_HDR				(V4L2_CID_DRIVER_BASE + 7)
...
static inline int valid_mode(u32 palette)
{
	return ((palette == V4L2_PIX_FMT_RGB565) ||
		...
		(palette == V4L2_PIX_FMT_Y10) ||
		(palette == V4L2_PIX_FMT_Y12) ||
...
static int mxc_v4l2_g_fmt(cam_data *cam, struct v4l2_format *f)
{
	...
	case V4L2_BUF_TYPE_VIDEO_OVERLAY:
		if(!strncmp(cam->sensor->name,"ar",2) || !strncmp(cam->sensor->name,"ov",2) || !strncmp(cam->sensor->name,"mt",2))
			return -EINVAL;
		else
		{
...
static int mxc_v4l2_s_fmt(cam_data *cam, struct v4l2_format *f)
{
	...
	int *width, *height;
	int err = 0;
	struct v4l2_format cam_fmt,check_fmt;
	...
	vidioc_int_g_fmt_cap(cam->sensor, &check_fmt);
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		err = vidioc_int_s_fmt_cap(cam->sensor, f);
		if (err) {
		...
		/* stride line limitation */
//		*width -= *width % 8;
//		*height -= *height % 8;		// Commented by e-Con
		...
		case V4L2_PIX_FMT_GREY:
			size = f->fmt.pix.width * f->fmt.pix.height* 3 / 2 ; /* Workaround for displaying Y8 in application as YUV420 */
			bytesperline = f->fmt.pix.width;
			break;
...
/* s_param called again if pixelformat is modified */
		if(check_fmt.fmt.pix.pixelformat!=f->fmt.pix.pixelformat)
		{
			...
			if (ifparm.u.bt656.clock_curr == 0)
				...
				if(!strcmp(cam->sensor->name, "ar1820") || !strcmp(cam->sensor->name, "ar0330"))
...
static int mxc_v4l2_g_ctrl(cam_data *cam, struct v4l2_control *c)
{
	...
	case V4L2_CID_GAIN:
		...
		break;
	case V4L2_CID_NOISE_FILTER:
		if (cam->sensor) {
			c->value = cam->noise_filter;
			status = vidioc_int_g_ctrl(cam->sensor, c);
			cam->noise_filter = c->value;
		} else {
			pr_err("ERROR: v4l2 capture: slave not found!\n");
			status = -ENODEV;
		}
	...
	case V4L2_CID_HDR:
        ...
        case V4L2_CID_FOCUS_TRIGGER:
                ...
                break;
        case V4L2_CID_TEMP_SENSE_AR1820_CTRL:
                if (cam->sensor) {
                       status = vidioc_int_g_ctrl(cam->sensor, c);
                } else {
                        pr_err("ERROR: v4l2 capture: slave not found!\n");
                ...
                break;
       case V4L2_CID_TEMP_SENSE_AR1820_READ:
                if (cam->sensor) {
                        status = vidioc_int_g_ctrl(cam->sensor, c);
                } else {
                        pr_err("ERROR: v4l2 capture: slave not found!\n");
	...
	case V4L2_CID_EXPOSURE_ABSOLUTE:
		...
		break;
        case V4L2_CID_EXPOSURE_ROI_WINDOW:
                 if (cam->sensor) {
			c->value = cam->ae_roi_window;
                         status = vidioc_int_g_ctrl(cam->sensor, c);
			cam->ae_roi_window = c->value;
                 } else {
                         pr_err("ERROR: v4l2 capture: slave not found!\n");
                         status = -ENODEV;
                 }
                 break;
        case V4L2_CID_EXPOSURE_ROI:
                 if (cam->sensor) {
			c->value = cam->ae_roi;
                         status = vidioc_int_g_ctrl(cam->sensor, c);
			cam->ae_roi = c->value;
			    ...
			    unsigned int ioctlnr, void *arg)
{
	struct video_device *dev = video_devdata(file);
	cam_data *cam = video_get_drvdata(dev);
	struct sensor_data *sdata = cam->sensor->priv;
	int retval = 0;
	unsigned long lock_flags;
...
//			V4L2_CAP_VIDEO_OVERLAY |
		...
		if(strncmp(cam->sensor->name, "ar",2) && strncmp(cam->sensor->name, "ov", 2) && strncmp(cam->sensor->name, "mt", 2) && strncmp(cam->sensor->name, "py", 2))
			cap->device_caps = cap->device_caps | V4L2_CAP_VIDEO_OVERLAY;

		cap->capabilities = cap->device_caps | V4L2_CAP_DEVICE_CAPS;
		strcpy(cap->card, cam->sensor->name);
		if(!strncmp(cam->sensor->name, "py", 2))
			sprintf(cap->bus_info, "SPI:%s", sdata->spi->modalias);
		else
			sprintf(cap->bus_info, "I2C:%s", sdata->i2c_client->adapter->name);
	...
	case VIDIOC_DBG_G_REGISTER:{
		...
		if (cam->sensor)
			retval = vidioc_int_dbg_g_register(cam->sensor, p);
		else {
			pr_err("ERROR: v4l2 capture: slave not found!\n");
			retval = -ENODEV;
		}
	...
	case VIDIOC_DBG_S_REGISTER:{
		struct v4l2_dbg_register *p = arg;
		if (cam->sensor)
			retval = vidioc_int_dbg_s_register(cam->sensor, p);
		else {
			pr_err("ERROR: v4l2 capture: slave not found!\n");
			retval = -ENODEV;
		}
...
static int init_camera_struct(cam_data *cam, struct platform_device *pdev)
{
	...
	*(cam->video_dev) = mxc_v4l_template;
	...
	cam->v2f.fmt.pix.sizeimage = 640 * 480 * 2;
